...
class AddNews extends Component
{
    ...
    public function updatedSelectPage() {
        $this->selection_categories = $this->all_categories->where('page_id', '=', $this->select_page);
        $this->select_category = null;
        
        $status = false;
        if ($this->select_page != null) {
            $status = true;
        }
        $this->dispatchBrowserEvent('open-input-field', ['status' => $status]);
    }
    ...
    {
        return view('livewire.admin-panel.add-news')->layout('layouts.admin');
    }

    public function validate_data() {
        $this->validate();
        $this->dispatchBrowserEvent('toggle-add-post-modal');
    }
    ...
    public function store_data() {
        ...
        $generator_rules = [
            'table' => 'posts',
            'length' => '12',
            'prefix' => date('ymd').$this->select_page,
